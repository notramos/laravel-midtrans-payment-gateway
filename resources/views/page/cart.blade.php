...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto p-6">
        <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 lg:py-8">
                    ...
                    <ol class="list-none p-0 inline-flex">
                        <li class="flex items-center">
                            <a href="{{ route('products.index') }}"
                                class="text-blue-600 hover:text-blue-800">Beranda</a>
            ...
            <!-- Alert Messages -->
            <div id="alertContainer" class="fixed top-8 left-1/2 transform -translate-x-1/2 z-50 space-y-2"></div>

            <!-- Empty Cart State -->
            <div class="text-center py-12 lg:py-16" id="empty-cart-state"
                style="{{ !$cart || $cart->cartItems->isEmpty() ? '' : 'display: none;' }}">
                <div class="max-w-md mx-auto px-4">
                    <div
                        class="w-20 h-20 lg:w-24 lg:h-24 mx-auto mb-4 lg:mb-6 bg-gray-100 rounded-full flex items-center justify-center">
                        <svg class="w-10 h-10 lg:w-12 lg:h-12 text-gray-400" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                    ...
                    <p class="text-gray-600 mb-6 text-sm lg:text-base">Belum ada produk dalam keranjang belanja Anda.
                        Mari mulai berbelanja!</p>
                    <a href="{{ route('products.index') }}"
            ...
            <div class="flex flex-col lg:grid lg:grid-cols-12 lg:gap-x-12 lg:items-start space-y-6 lg:space-y-0"
                id="cart-content" style="{{ !$cart || $cart->cartItems->isEmpty() ? 'display: none;' : '' }}">
                <!-- Cart Items -->
                <div class="lg:col-span-8 order-2 lg:order-1">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="bg-gray-50 px-4 lg:px-6 py-3 lg:py-4 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                <h2 class="text-base lg:text-lg font-semibold text-gray-900">
                                    Item dalam Keranjang (<span
                                        id="cart-items-count">{{ $cart && $cart->cartItems ? $cart->cartItems->count() : 0 }}</span>)
                                </h2>
                        ...
                        <!-- Cart Items List -->
                        <div class="divide-y divide-gray-200 space-y-4" id="cart-items-container">
                            @if ($cart && $cart->cartItems && $cart->cartItems->count() > 0)
                                @foreach ($cart->cartItems as $item)
                                    <div class="p-4 lg:p-6" data-item-id="{{ $item->id }}"
                                        ...
                                        <div
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="space-y-3">
                                                    <div>
                                                        <h3
                                                            class="text-base lg:text-lg font-semibold text-gray-900 mb-1">
                                                            {{ $item->product->name }}
                                                        </h3>
                                                    ...
                                                    @if ($item->panjang && $item->lebar)
                                                        <div
                                                            class="bg-gray-50 border border-gray-200 rounded p-2 text-xs lg:text-sm text-gray-700">
                                                    ...
                                                    @if ($item->design_notes)
                                                        <div
                                                            class="bg-blue-50 border border-blue-200 rounded-lg p-2 lg:p-3">
                                                            <div class="flex items-start">
                                                                <svg class="w-3 h-3 lg:w-4 lg:h-4 text-blue-600 mt-0.5 mr-2 flex-shrink-0"
                                                    ...
                                                    @if ($item->design_file)
                                                        <div class="flex items-center space-x-2">
                                                            <svg class="w-3 h-3 lg:w-4 lg:h-4 text-green-600"
                                                                fill="none" stroke="currentColor"
                                                                viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2"
                                                                ...
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2" d="M9 12l2 2 4-4"></path>
                                                            </svg>
                                                    ...
                                                    <div class="space-y-3">
                                                        ...
                                                        <div class="text-sm text-gray-600">
                                                            ...
                                                            <span>/{{ $item->product->isUkuranBased() ? 'meter' : 'pcs' }}</span>
                                                        </div>
                                                        ...
                                                        <div
                                                            ...
                                                            <div
                                                                ...
                                                                <div
                                                                    ...
                                                                    <button
                                                                        class="quantity-btn minus-btn px-2 lg:px-3 py-1 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-l-lg transition-colors"
                                                                        data-item-id="{{ $item->id }}"
                                                                        data-action="decrease"
                                                                        data-current-qty="{{ $item->quantity }}"
                                                                        {{ $item->quantity <= 1 ? 'disabled' : '' }}>
                                                                        <svg class="w-3 h-3 lg:w-4 lg:h-4"
                                                                            fill="none" stroke="currentColor"
                                                                            viewBox="0 0 24 24">
                                                                    ...
                                                                    <span
                                                                        class="quantity-display px-3 lg:px-4 py-1 text-sm font-medium text-gray-900 bg-gray-50 border-l border-r border-gray-300 min-w-[3rem] text-center"
                                                                        id="quantity-{{ $item->id }}">
                                                                        {{ $item->quantity }}
                                                                    </span>

                                                                    <button
                                                                        class="quantity-btn plus-btn px-2 lg:px-3 py-1 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-r-lg transition-colors"
                                                                        data-item-id="{{ $item->id }}"
                                                                ...
                                                                <button
                                                                    class="remove-item-btn sm:hidden p-2 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition-colors"
                                                                    data-item-id="{{ $item->id }}">
                                                            ...
                                                            @php
                                                                $isUkuranBased = $item->product->isUkuranBased();
                                                                $productPrice = $item->product->price;
                                                                $quantity = $item->quantity;
                                                                $panjang = $item->panjang ?? 1;
                                                                $lebar = $item->lebar ?? 1;
                                                                $subtotal = $isUkuranBased
                                                                    ? $productPrice * $quantity * $panjang * $lebar
                                                            ...
                                                            <div
                                                                ...
                                                                <button
                                                                    class="remove-item-btn hidden sm:block p-2 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition-colors"
                                                                    data-item-id="{{ $item->id }}">
                                                                    <svg class="w-4 h-4 lg:w-5 lg:h-5" fill="none"
                                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round"
                                                                            stroke-linejoin="round" stroke-width="2"
                                                                            d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                        ...
                        <div class="px-4 lg:px-6 py-4 space-y-4">
                            ...
                            <div class="space-y-3">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600">Subtotal (<span
                                            id="summary-items-count">{{ $cart && $cart->cartItems ? $cart->cartItems->sum('quantity') : 0 }}</span>
                                        item)</span>
                                    <span class="font-medium" id="summary-subtotal">Rp
                                ...
                                <div class="border-t border-gray-200 pt-3">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="text-lg lg:text-xl font-bold text-blue-600" id="summary-total">Rp
                                            {{ number_format($total ?? 0, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-3 pt-4">
                                <button id="checkout-btn"
                                    class="w-full bg-blue-600 text-white text-center cursor-pointer py-2.5 lg:py-3 px-4 rounded-lg font-semibold hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-200 flex items-center justify-center text-sm lg:text-base">
                                    <span class="checkout-btn-text">
                                        <svg class="w-4 h-4 lg:w-5 lg:h-5 mr-2 inline" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <span class="checkout-btn-loading hidden">
                                        <svg class="w-4 h-4 lg:w-5 lg:h-5 mr-2 inline animate-spin" fill="none"
                                            viewBox="0 0 24 24">
    ...
    <div id="loading-overlay"
        class=" fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center z-50 hidden">
    ...
    <div id="confirmation-modal"
        class="fixed inset-0 bg-transparent bg-opacity-10 flex items-center justify-center z-50 hidden">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function showAlert(message, type = 'success') {
                ...
                if (!alertContainer) {
                    ...
                    alertContainer.className = 'fixed top-8 left-1/2 transform -translate-x-1/2 z-50 space-y-2';
                ...
                alert.className =
                    `${alertClass} px-4 py-3 rounded-lg shadow-md flex items-center w-full max-w-md mx-auto animate-fade`;
                alert.innerHTML = `
    ...
    `;
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            if (calculationDisplay) {
                                if (data.is_ukuran_based) {
                                    calculationDisplay.textContent =
                                        `${newQty} × ${data.panjang}m × ${data.lebar}m × ${data.item_price_formatted}`;
                                } else {
                ...
                fetch('/cart/checkout', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            if (data.redirect) {
                                showAlert('Checkout berhasil! Anda akan diarahkan ke halaman pembayaran.',
                                    'success');
                                setTimeout(() => {
            ...
            window.addEventListener('offline', function() {
                showAlert('Koneksi internet terputus. Perubahan akan disimpan saat koneksi kembali.',
                    'error');
            });
